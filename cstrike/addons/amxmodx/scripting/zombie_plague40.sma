...
new const ZP_CUSTOMIZATION_FILE[] = "biohazard.ini"
new const ZP_EXTRAITEMS_FILE[] = "biohazard_extras.ini"
new const ZP_ZOMBIECLASSES_FILE[] = "biohazard_zmclasses.ini"
...
// Plugin Version
new const PLUGIN_VERSION[] = "Biohazard Mod"
...
public plugin_cfg()
{
	...
	// Execute config file (biohazard.cfg)
	server_cmd("exec %s/biohazard.cfg", cfgdir)
...
public logevent_round_end()
{
	...
	{
		// Human team wins
		set_hudmessage(0, 155, 200, HUD_EVENT_X, HUD_EVENT_Y, 0, 0.0, 3.0, 2.0, 1.0)
	...
	{
		// Zombie team wins
		set_hudmessage(255, 0, 0, HUD_EVENT_X, HUD_EVENT_Y, 0, 0.0, 3.0, 2.0, 1.0)
	...
	{
		// No one wins
		set_hudmessage(155, 255, 0, HUD_EVENT_X, HUD_EVENT_Y, 0, 0.0, 3.0, 2.0, 1.0)
...
public clcmd_buyammo(id)
{
	...
	if (g_zombie[id])
	{
		client_print(id, print_chat,  "^^2[Biohazard]^^7 %L", id, "CMD_HUMAN_ONLY")
		return PLUGIN_HANDLED;
	}
	...
	if (g_ammopacks[id] < 1)
	{
		client_print(id, print_chat,  "^^2[Biohazard]^^7 %L", id, "NOT_ENOUGH_AMMO")
		return PLUGIN_HANDLED;
	}
	...
	client_print(id, print_chat,  "^^2[Biohazard]^^7 %L", id, "AMMO_BOUGHT")
...
show_menu_game(id)
{
	...
	if (get_pcvar_num(cvar_zclasses))
		len += formatex(menu[len], charsmax(menu) - len, "\r[3]\w %L^n", id,"MENU_ZCLASS")
	else
	...
	if (!g_isalive[id] || !get_pcvar_num(cvar_blocksuicide) || (userflags & g_access_flag[ACCESS_ADMIN_MENU]))
		len += formatex(menu[len], charsmax(menu) - len, "\r[5]\w %L^n^n", id, "MENU_SPECTATOR")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\d[5] %L^n^n", id, "MENU_SPECTATOR")
...
show_menu_extras(id)
{
	...
	{
		client_print(id, print_chat,  "^^2[Biohazard]^^7 %L", id ,"CMD_NOT_EXTRAS")
		menu_destroy(menuid)
		return;
	}
	...
	switch (key)
	{
		case 0: // Buy Weapons
		{
			...
			if (get_pcvar_num(cvar_buycustom))
			{
				...
				WPN_AUTO_ON = 0
				client_print(id, print_chat,  "^^2[Biohazard]^^7 %L", id, "BUY_ENABLED")
				
				// Show menu if player hasn't yet bought anything
				if (g_canbuy[id]) show_menu_buy1(id)
			}
			else
				client_print(id, print_chat,  "^^2[Biohazard]^^7 %L", id, "CMD_NOT")
		}
		case 1: // Extra Items
		{
			// Extra items enabled?
		...
		case 2: // Zombie Classes
		{
			...
			if (get_pcvar_num(cvar_zclasses))
				...
				client_print(id, print_chat,  "^^2[Biohazard]^^7 %L", id, "CMD_NOT_ZCLASSES")
		}
		case 3: // Unstuck
		{
			...
			if (g_isalive[id])
			{
				if (is_player_stuck(id))
				{
					...
					client_print(id, print_chat,  "^^2[Biohazard]^^7 %L", id, "CMD_NOT_STUCK")
			}
			else
				client_print(id, print_chat,  "^^2[Biohazard]^^7 %L", id, "CMD_NOT")
		}
		...
		case 8: // Admin Menu
		{
			...
			if (get_user_flags(id) & g_access_flag[ACCESS_ADMIN_MENU])
				...
				client_print(id, print_chat,  "^^2[Biohazard]^^7 %L", id, "CMD_NOT_ACCESS")
		}
	}
...
buy_extra_item(id, itemid, ignorecost = 0)
{
	...
	{
		client_print(id, print_chat,  "^^2[Biohazard]^^7 %L", id, "CMD_NOT_CANTUSE")
		return;
	}
...
public menu_zclass(id, menuid, item)
{
	...
	client_print(id, print_chat,  "^^2[Biohazard]^^7 %L: %s", id, "ZOMBIE_SELECT", name)
	client_print(id, print_chat,  "^^2[Biohazard]^^7 %L: %d %L: %d %L: %d %L: %d%%", id, "ZOMBIE_ATTRIB1", ArrayGetCell(g_zclass_hp, g_zombieclassnext[id]), id, "ZOMBIE_ATTRIB2", ArrayGetCell(g_zclass_spd, g_zombieclassnext[id]),
...
make_a_zombie(mode, id)
{
	...
	if ((mode == MODE_NONE && (!get_pcvar_num(cvar_preventconsecutive) || g_lastmode != MODE_SURVIVOR) && random_num(1, get_pcvar_num(cvar_survchance)) == get_pcvar_num(cvar_surv) && iPlayersnum >= get_pcvar_num(cvar_survminplayers)) || mode == MODE_SURVIVOR)
	{
		...
		set_hudmessage(0, 155, 255, HUD_EVENT_X, HUD_EVENT_Y, 1, 0.0, 5.0, 1.0, 1.0)
	...
	{		
		...
		set_hudmessage(0, 255, 0, HUD_EVENT_X, HUD_EVENT_Y, 1, 0.0, 5.0, 1.0, 1.0)
	...
	else if ((mode == MODE_NONE && (!get_pcvar_num(cvar_preventconsecutive) || g_lastmode != MODE_MULTI) && random_num(1, get_pcvar_num(cvar_multichance)) == get_pcvar_num(cvar_multi) && floatround(iPlayersnum*get_pcvar_float(cvar_multiratio), floatround_ceil) >= 2 && floatround(iPlayersnum*get_pcvar_float(cvar_multiratio), floatround_ceil) < iPlayersnum && iPlayersnum >= get_pcvar_num(cvar_multiminplayers)) || mode == MODE_MULTI)
	{
		...
		set_hudmessage(255, 155, 0, HUD_EVENT_X, HUD_EVENT_Y, 1, 0.0, 5.0, 1.0, 1.0)
	...
	&& iPlayersnum-(get_pcvar_num(cvar_plaguesurvnum)+get_pcvar_num(cvar_plaguenemnum)+floatround((iPlayersnum-(get_pcvar_num(cvar_plaguenemnum)+get_pcvar_num(cvar_plaguesurvnum)))*get_pcvar_float(cvar_plagueratio), floatround_ceil)) >= 1 && iPlayersnum >= get_pcvar_num(cvar_plagueminplayers)) || mode == MODE_PLAGUE)
	{
		...
		set_hudmessage(0, 100, 255, HUD_EVENT_X, HUD_EVENT_Y, 1, 0.0, 5.0, 1.0, 1.0)
	...
	else
	{
		...
		if (g_nemround)
		{
			...
			set_hudmessage(255, 0, 0, HUD_EVENT_X, HUD_EVENT_Y, 1, 0.0, 5.0, 1.0, 1.0)
			show_hudmessage(0, "%L", LANG_PLAYER, "NOTICE_NEMESIS", g_playername[forward_id])
		...
		else
		{
			...
			set_hudmessage(0, 255, 0, HUD_EVENT_X, HUD_EVENT_Y, 0, 0.0, 5.0, 1.0, 1.0)
			show_hudmessage(0, "%L",LANG_PLAYER, "NOTICE_FIRST", g_playername[forward_id])
...
humanme(id, survivor, silentmode)
{
	...
	else
	{
		...
		if (!silentmode)
		{
			...
			set_hudmessage(0, 155, 255, HUD_INFECT_X, HUD_INFECT_Y, 0, 0.0, 5.0, 1.0, 1.0)
			show_hudmessage(0, "%L", LANG_PLAYER, "NOTICE_ANTIDOTE", g_playername[id])
		}
	}
...
{
	// Show mod info
	client_print(0, print_chat,  "^x01**** ^x04%s^x01 ****", g_modname)
	client_print(0, print_chat,  "^^2[Biohazard]^^7 %L", LANG_PLAYER, "NOTICE_INFO1")
	if (!get_pcvar_num(cvar_infammo)) client_print(0, print_chat,  "^^2[Biohazard]^^7 %L", LANG_PLAYER, "NOTICE_INFO2")
	
	// Show T-virus HUD notice
	set_hudmessage(0, 125, 200, HUD_EVENT_X, HUD_EVENT_Y, 0, 0.0, 3.0, 2.0, 1.0)
...
check_round(leaving_player)
{
	...
	if (g_zombie[leaving_player] && fnGetZombies() == 1)
	{
		...
		client_print(0, print_chat,  "^^2[Biohazard]^^7 %L", LANG_PLAYER, "LAST_ZOMBIE_LEFT", g_playername[id])
	...
	else if (!g_zombie[leaving_player] && fnGetHumans() == 1)
	{
		...
		client_print(0, print_chat,  "^^2[Biohazard]^^7 %L", LANG_PLAYER, "LAST_HUMAN_LEFT", g_playername[id])
...
public ShowHUD(taskid)
{
	...
	if (id != ID_SHOWHUD)
	{
		...
		set_hudmessage(255, 255, 255, 0.01, 0.21, 1, 6.0, 1.1, 0.0, 0.0, -1)
		ShowSyncHudMsg(ID_SHOWHUD, g_MsgSync2, "%L %s^n=- Health: %d^n=- %L %s ^n=- %L %d", ID_SHOWHUD, "SPECTATING", g_playername[id], pev(id, pev_health), ID_SHOWHUD, "CLASS_CLASS", class, ID_SHOWHUD, "AMMO_PACKS1", g_ammopacks[id])
	}
	else
	{
		// Show health, class and ammo packs
		set_hudmessage(red, green, blue, 0.01, 0.21, 1, 6.0, 1.1, 0.0, 0.0, -1)
		ShowSyncHudMsg(ID_SHOWHUD, g_MsgSync2, "=- Biohazard^n=- %L: %d^n=- %L %s^n=- %L %d", id, "ZOMBIE_ATTRIB1", pev(ID_SHOWHUD, pev_health), ID_SHOWHUD, "CLASS_CLASS", class, ID_SHOWHUD, "AMMO_PACKS1", g_ammopacks[ID_SHOWHUD])
	}
}
